...
public class BrowserMobProxyClient {
	...
	public int getPortUsingUpstreamProxy(String upStreamProxyServer) {
		ClientResponse response = service.queryParam("httpProxy", upStreamProxyServer).post(ClientResponse.class);
		return Integer.parseInt(response.getEntity(String.class).substring(PORT_BEGININDEX, PORT_ENDINDEX));
	}

	public boolean addHeader(Map<String, String> headers) throws IOException {
		ClientResponse response = service.path(Integer.toString(port)).path("headers").post(ClientResponse.class, mapper.writeValueAsString(headers));
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}

	public boolean autoBasicAuthorization(String domain, String username, String password) throws IOException {
		Map<String, String> auth = new HashMap<String, String>();
		auth.put("username", username);
		auth.put("password", password);

		ClientResponse response = service.path(Integer.toString(port)).path("auth").path("basic").path(domain)
	...
	public boolean blacklistRequests(String pattern, int responseCode) {
		...
		ClientResponse response = service.path(Integer.toString(port)).path("blacklist").put(ClientResponse.class, formData);
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}

	public boolean clearDNSCache() {
		ClientResponse response = service.path(Integer.toString(port)).path("dns").path("cache").delete(ClientResponse.class);
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}

	public Har getHar() throws IOException {
		ClientResponse response = service.path(Integer.toString(port)).path("har").get(ClientResponse.class);
	...
	public int getPort() {
		...
		String responseBody = response.getEntity(String.class);
		int actPort = Integer.parseInt(responseBody.substring(PORT_BEGININDEX, PORT_ENDINDEX));
		return actPort;
	}
	...
	public int getPort(int port) {
		...
		int newPort = Integer.parseInt(response.getEntity(String.class).substring(PORT_BEGININDEX, PORT_ENDINDEX));
		return newPort;
	}
	...
	public boolean enableLimiter(boolean status) {
		...
		ClientResponse response = service.path(Integer.toString(port)).path("limit").put(ClientResponse.class, formData);
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}
	...
	public boolean newHar(String initialPageRef, boolean captureContent, boolean captureHeaders, boolean captureBinaryContent) {
		MultivaluedMap<String, String> formData = new MultivaluedMapImpl();
		...
		ClientResponse response = service.path(Integer.toString(port)).path("har").put(ClientResponse.class, formData);
	...
	public boolean newPage(String pageRef) {
		...
		ClientResponse response = service.path(Integer.toString(port)).path("har").path("pageRef").put(ClientResponse.class, formData);
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}

	public boolean remapHost(Map<String, String> hosts) throws IOException {
		ClientResponse response = service.path(Integer.toString(port)).path("hosts").post(ClientResponse.class, mapper.writeValueAsString(hosts));
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}
	...
	public boolean rewriteUrl(String matchRegex, String replace) {
		...
		ClientResponse response = service.path(Integer.toString(port)).path("rewrite").put(ClientResponse.class, formData);
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}
	...
	public boolean setConnectionTimeout(long connectionTimeout) {
		...
		ClientResponse response = service.path(Integer.toString(port)).path("timeout").put(ClientResponse.class, formData);
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}
	...
	public boolean setRetryCount(int retrycount) {
		...
		ClientResponse response = service.path(Integer.toString(port)).path("retry").put(ClientResponse.class, formData);
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}
	...
	public boolean stop() {
		ClientResponse response = service.path(Integer.toString(port)).delete(ClientResponse.class);
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}

	public boolean waitForNetworkTrafficToStop(long quietPeriodInMs, long timeoutInMs) {
		MultivaluedMap<String, String> formData = new MultivaluedMapImpl();
		formData.add("quietPeriodInMs", Long.toString(quietPeriodInMs));
		formData.add("timeoutInMs", Long.toString(timeoutInMs));
		ClientResponse response = service.path(Integer.toString(port)).path("wait").put(ClientResponse.class, formData);
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}
	...
	public boolean whitelistRequests(String patterns, int responseCode) {
		...
		formData.add("status", Integer.toString(responseCode));
		ClientResponse response = service.path(Integer.toString(port)).path("whitelist").put(ClientResponse.class, formData);
		return (response.getStatus() == HTTP_STATUS_CODE_200);
	}
